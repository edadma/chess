...
class SimpleEngine extends Engine {
  ...
  private def evaluatePosition(board: Board, side: Side): Int = {
  ...
  private def evaluatePawnStructure(board: Board, side: Side): Int = {
    var score = 0
  ...
  private def evaluateKingSafety(board: Board, side: Side): Int = {
    ...
    board.getPiecesByType(Set(PieceType.KING), side).foreach { kingSquare =>
      ...
      val attackedSquaresAroundKing = List(
        kingSquare - 9, kingSquare - 8, kingSquare - 7,
        kingSquare - 1,                 kingSquare + 1,
        kingSquare + 7, kingSquare + 8, kingSquare + 9
      ).count(square =>
        square >= 0 && square < 64 &&
